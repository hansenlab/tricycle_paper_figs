...
theta2.hat <- as.numeric(circular::coord2rad(pca.o$x[, 2:3]))
theta2.hat.shifted <- ifelse(theta2.hat < theta2.hat[1], 2*pi - theta2.hat[1] + theta2.hat, theta2.hat - theta2.hat[1])


tmp.df <- data.frame(pc1 = pca.o$x[,1], pc2 = pca.o$x[,2], 
										 pc3= pca.o$x[,3], pc4 = pca.o$x[,4],
										 timepoints = timepoints, theta = theta.hat.shifted, theta2 = theta2.hat.shifted)
...
c.p <- ggplot(tmp.df, aes(x = pc1, y = pc3)) +
	geom_scattermore(shape = 1, pointsize = 2.01, alpha = 0.7) +
	labs(title = "PC1 and PC3", x = sprintf("PC1 (%2.1f%%)", imp[2,1]*100), y = sprintf("PC3 (%2.1f%%)", imp[2,3]*100)) +
	# annotate("point", x = 0, 
	# 				 y = 0, shape = 16, size = 1, alpha = 0.8) + 
	# annotate("segment", x = 0, xend =  4 ,
	# 				 y = 0, yend = 0, linetype = "dashed", alpha = 0.8) + 
...
d.p <- ggplot(tmp.df, aes(x = pc1, y = pc4)) +
	geom_scattermore(shape = 1, pointsize = 2.01, alpha = 0.7) +
	labs(title = "PC1 and PC4", x = sprintf("PC1 (%2.1f%%)", imp[2,1]*100), y = sprintf("PC4 (%2.1f%%)", imp[2,4]*100)) +
	# annotate("point", x = 0, 
	# 				 y = 0, shape = 16, size = 1, alpha = 0.8) + 
	# annotate("segment", x = 0, xend =  4 ,
	# 				 y = 0, yend = 0, linetype = "dashed", alpha = 0.8) + 
...
e.p <- ggplot(tmp.df, aes(x = pc2, y = pc3)) +
	geom_scattermore(shape = 1, pointsize = 2.01, alpha = 0.7) +
	labs(title = "PC2 and PC3", x = sprintf("PC2 (%2.1f%%)", imp[2,2]*100), y = sprintf("PC3 (%2.1f%%)", imp[2,3]*100)) +
	# annotate("point", x = 0, 
	# 				 y = 0, shape = 16, size = 1, alpha = 0.8) + 
	# annotate("segment", x = 0, xend =  4 ,
	# 				 y = 0, yend = 0, linetype = "dashed", alpha = 0.8) + 
...
f.p <- ggplot(tmp.df, aes(x = pc2, y = pc4)) +
	geom_scattermore(shape = 1, pointsize = 2.01, alpha = 0.7) +
	labs(title = "PC2 and PC4", x = sprintf("PC2 (%2.1f%%)", imp[2,2]*100), y = sprintf("PC4 (%2.1f%%)", imp[2,4]*100)) +
	# annotate("point", x = 0, 
	# 				 y = 0, shape = 16, size = 1, alpha = 0.8) + 
	# annotate("segment", x = 0, xend =  4 ,
	# 				 y = 0, yend = 0, linetype = "dashed", alpha = 0.8) + 
...
g.p <- ggplot(tmp.df, aes(x = timepoints, y = theta)) +
	geom_scattermore(shape = 1, pointsize = 2.01, alpha = 0.7) +
	scale_x_continuous(breaks = c(0, 0.5, 1, 1.5, 2) * pi, labels = c(0, str_c(seq(0.5, 2, 0.5), "\u03C0")), limits = c(0, 2) * pi)  +
...
h.p <- ggplot(tmp.df, aes(x = timepoints, y = theta2)) +
	geom_scattermore(shape = 1, pointsize = 2.01, alpha = 0.7) +
	scale_x_continuous(breaks = c(0, 0.5, 1, 1.5, 2) * pi, labels = c(0, str_c(seq(0.5, 2, 0.5), "\u03C0")), limits = c(0, 2) * pi)  +
	scale_y_continuous(breaks = c(0, 0.5, 1, 1.5, 2) * pi, labels = c(0, str_c(seq(0.5, 2, 0.5), "\u03C0")), limits = c(0, 2) * pi)  +
	labs(title = "Recovered timepoints using PC2-3", x = "True timepoint \u03B8", y = "Recovered timepoint \u03B8") +
	theme(legend.position = c(0, 1),
				legend.justification = c(0, 1),
				legend.direction="horizontal",
				axis.title.y = element_text(size = 8),
