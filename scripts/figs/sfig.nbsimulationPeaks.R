...
mp <- plot_grid(plotlist = lapply(seq_along(c(2, 3, 5, 10, 50)), function(i) {
	plotPCA(pca = pca.lo[[i]], npeak = c(2, 3, 5, 10, 50)[i], gene_xlims = gene_xlims, gene_ylims =  gene_ylims , cell_xlims = cell_xlims, cell_ylims = cell_ylims)
}),
nrow = 5, ncol = 1, label_size = 10, labels = "auto", align = "v", axis = "lr")


save_plot(here::here("figs", "sfigs", "sfig.nbsimulationPeaks.pdf"), mp,
					base_height = 2, base_width = 2*1.2, nrow = 5, ncol = 4, device = cairo_pdf)
