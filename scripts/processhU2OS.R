...
### make plotting data
names(colData(sce.o))
sel.col <- c("TotalUMIs", "top2a", "smc4", "pct0", "tricyclePosition", "cyclone", "SeuratCC", "CCStage", "sample", "cell_type", "age", "celltype",  "StageAJC", "Green530","Red585", "centered.Green530", "centered.Red585", "fucci_time")
sel.col[!(sel.col %in% names(colData(sce.o)))]

colData.df <- colData(sce.o)[, sel.col[(sel.col %in% names(colData(sce.o)))]]
colData.df$StageAJC[which(colData.df$StageAJC == "S-ph")] <- "S"
colData.df$StageAJC <- factor(colData.df$StageAJC, levels = c("G1", "S", "G2M"))
...
reducedDims.l <- list(go.pca = go.pca.m, umap = reducedDim(sce.o, "UMAP.s"), tricycleEmbedding = reducedDim(sce.o, "tricycleEmbedding"),
											fucci =  data.frame(Green530 = sce.o$Green530, Red585 = sce.o$Red585))
