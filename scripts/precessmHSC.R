...
sce.o <- estimate_cycle_stage(sce.o, gname = gene, batch.v = sce.o$age, exprs_values = "log.s", gname.type = "SYMBOL", species = species)
...
### make plotting data
names(colData(sce.o))
sel.col <- c("TotalUMIs", "top2a", "smc4", "pct0", "tricyclePosition", "cyclone", "SeuratCC", "CCStage", "sample", "cell_type", "age", "celltype")
sel.col[!(sel.col %in% names(colData(sce.o)))]

colData.df <- colData(sce.o)[, sel.col[(sel.col %in% names(colData(sce.o)))]]
names(colData.df)[names(colData.df) == "celltype"] <- "cell_type"
