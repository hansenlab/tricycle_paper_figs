...
### make plotting data
names(colData(sce.o))
sel.col <- c("TotalUMIs", "top2a", "smc4", "pct0", "tricyclePosition", "cyclone", "SeuratCC", "CCStage", "sample", "cell_type", "age", "celltype",  "gfp.median.log10sum.adjust", "rfp.median.log10sum.adjust")
sel.col[!(sel.col %in% names(colData(sce.o)))]

colData.df <- colData(sce.o)[, sel.col[(sel.col %in% names(colData(sce.o)))]]
colData.df$Hsiao.theta <- as.numeric(coord2rad(cbind(sce.o$rfp.median.log10sum.adjust, sce.o$gfp.median.log10sum.adjust)))
...
reducedDims.l <- list(go.pca = go.pca.m, umap = reducedDim(sce.o, "UMAP.s"), tricycleEmbedding = reducedDim(sce.o, "tricycleEmbedding"),
											fucci =  data.frame(mCherry = sce.o$rfp.median.log10sum.adjust, EGFP = sce.o$gfp.median.log10sum.adjust))
